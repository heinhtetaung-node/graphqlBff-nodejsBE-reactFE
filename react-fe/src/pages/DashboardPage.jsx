...
import { GET_ME, MY_APPLICATIONS, MY_JOBS, APPLICATIONS_BY_JOB } from '../graphql/queries';
...
function JobApplications({ jobId }) {
  ...
  if (loading) return <div style={{ padding: '8px 0', color: '#666' }}>Loading applications...</div>;

  const apps = data?.applicationsByJob?.applications || [];
  if (apps.length === 0) return <div style={{ padding: '8px 0', color: '#666' }}>No applications yet.</div>;

  return (
    <div style={{ marginTop: 8 }}>
      {apps.map((app) => (
        <div key={app.id} style={{ padding: '8px 12px', background: '#f9f9f9', borderRadius: 4, marginBottom: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <span style={{ color: '#555' }}>Applicant ID: {app.userId?.slice(0, 8)}...</span>
            {app.coverLetter && <p style={{ margin: '4px 0 0', fontSize: 13, color: '#777' }}>{app.coverLetter.slice(0, 100)}{app.coverLetter.length > 100 ? '...' : ''}</p>}
          </div>
      ...
      {user?.role === 'TALENT_HUNTER' && (
        <div className="card">
          ...
          {!myJobsData?.myJobs?.jobs?.length ? (
            <p>You haven't posted any jobs yet. <Link to="/jobs/new">Post your first job</Link></p>
          ) : (
            myJobsData.myJobs.jobs.map((job) => (
              <div key={job.id} style={{ padding: '12px 0', borderBottom: '1px solid #eee' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <Link to={`/jobs/${job.id}`} style={{ fontWeight: 600, color: '#2563eb', textDecoration: 'none' }}>{job.title}</Link>
                    <div style={{ fontSize: 13, color: '#666', marginTop: 2 }}>
                      {job.location && <span>{job.location}</span>}
                      {job.jobType && <span style={{ marginLeft: 8 }}>&middot; {job.jobType}</span>}
                      {job.salaryRange && <span style={{ marginLeft: 8 }}>&middot; {job.salaryRange}</span>}
                    </div>
                  </div>
